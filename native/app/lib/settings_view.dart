...
class _SettingsViewState extends State<SettingsView> {
  ...
  List<Widget> modelSettings(BuildContext context) => [
    ...
    if (s.backendFallbackReasons.isNotEmpty)
      Text('Automatic fallback: ${s.backendFallbackReasons.join('; ')}'),
    section('Generation'),
    ...
    Slider(
      ...
      onChanged: s.busy
          ? null
          : (v) =>
                s.setSampling(temperature: s.temperature, repetitionPenalty: v),
    ),
    const Text(
    ...
    DropdownButtonFormField<String>(
      ...
      items: [
        ...
        if (device != 'auto' && !s.devices.any((d) => d['id'] == device))
          DropdownMenuItem(value: device, child: Text('$device (unavailable)')),
    ...
    TextField(
      ...
      enabled: !s.busy,
      keyboardType: TextInputType.number,
      decoration: const InputDecoration(labelText: 'Reply budget tokens'),
    ),
        ...
        onPressed: s.busy
            ...
            : () async {
                ...
                if (contextTokens == null ||
                    replyTokens == null ||
                    s.profile.limitsError(contextTokens, replyTokens) != null) {
                  s.setNotice(
  ...
  List<Widget> advancedSettings(BuildContext context) => [
    ...
    if (s.desktop) ...[
      ...
      TextField(
        ...
        keyboardType: TextInputType.number,
        decoration: const InputDecoration(labelText: 'Local API port'),
      ),
      SwitchListTile(
        title: const Text('OpenAI-compatible local API'),
        subtitle: Text(s.apiServer?.address ?? 'Off · localhost only'),
    ...
    section('Experimental inference'),
    SwitchListTile(
