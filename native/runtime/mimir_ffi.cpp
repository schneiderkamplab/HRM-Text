...
struct Engine {
    ...
    std::string system; int context=0;
    std::thread worker;
    ...
    void execute(const Json & c) {
        ...
        auto full=history(c);chat->recover();chat->restore_history(full);
    ...
    void run() {
        for(;;) {
            ...
            try{static std::once_flag once;std::call_once(once,[]{ggml_backend_load_all();llama_backend_init();});execute(c);}catch(const std::exception & e){emit({{"type","error"},{"message",e.what()}});}
