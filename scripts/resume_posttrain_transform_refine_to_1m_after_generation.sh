...
AUDIT_ROOT="${AUDIT_ROOT:-logs/posttrain_transform_refine_generation/audits_to_1m_resume_$(date +%Y%m%dT%H%M%S)}"
...
for idx in "${!GPUS[@]}"; do
  wait_for_server "$(port_for_index "$idx")"
done
...
echo "Done. Generated rows: $GENERATED_ROOT"
