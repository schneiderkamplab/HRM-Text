...
MODEL_NAME="${MODEL_NAME:-openai/gemma-4-e4b-nemotron-swe-judge}"
...
RUN_ROOT="${RUN_ROOT:-logs/data_audits/nemotron_swe_repaired_20260828_v4}"
...
(( ${#GPU_IDS[@]} > 0 )) || { echo "GPUS must name at least one GPU" >&2; exit 2; }
...
for slot in "${!GPU_IDS[@]}"; do
  gpu="${GPU_IDS[$slot]}"
  port=$((PORT_BASE + slot)); deadline=$((SECONDS + 900))
  until curl -fsS "http://127.0.0.1:${port}/v1/models" >/dev/null 2>&1; do
    (( SECONDS <= deadline )) || { echo "GPU${gpu} server startup timed out" >&2; exit 1; }
    kill -0 "${SERVER_PIDS[$slot]}" 2>/dev/null || { echo "GPU${gpu} server exited" >&2; exit 1; }
    sleep 2
  done
done

for ((offset=0; offset<PARTITIONS; offset+=${#GPU_IDS[@]})); do
  WORKER_PIDS=()
  ACTIVE_PARTITIONS=()
  for slot in "${!GPU_IDS[@]}"; do
    partition=$((offset + slot))
    (( partition < PARTITIONS )) || continue
    gpu="${GPU_IDS[$slot]}"
    "$CLIENT_PYTHON" scripts/dfm10_quality_audit.py audit \
      --samples "$SAMPLES" --output "$RUN_ROOT/partitions/partition_${partition}.jsonl" \
      --base-url "http://127.0.0.1:$((PORT_BASE + slot))/v1" --model "$MODEL_NAME" \
      --partitions "$PARTITIONS" --partition-index "$partition" --concurrency "$CONCURRENCY" \
      --max-tokens 512 --retries 3 --resume --progress-interval 25 \
      >"$RUN_ROOT/partitions/partition_${partition}.log" 2>&1 &
    WORKER_PIDS+=("$!")
    ACTIVE_PARTITIONS+=("$partition")
  done
  status=0
  for slot in "${!WORKER_PIDS[@]}"; do
    wait "${WORKER_PIDS[$slot]}" || {
      echo "Partition ${ACTIVE_PARTITIONS[$slot]} failed" >&2
      status=1
    }
  done
  WORKER_PIDS=()
...
"$CLIENT_PYTHON" scripts/dfm10_quality_audit.py merge \
  ...
  --partitions "$PARTITIONS" --output "$RUN_ROOT/nemotron_swe_repaired_quality_audit.jsonl"
