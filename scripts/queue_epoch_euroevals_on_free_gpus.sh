...
QUEUE_ROOT="${QUEUE_ROOT:-logs/euroeval/queued_epoch_euroevals_$(date +%Y%m%dT%H%M%S)}"
...
jobs=(
  "dfm_L	1	checkpoints/dfm/L	epoch_1	1	hrm-dfm-L	logs/euroeval/dfm_L_epoch_checkpoints	DFM L	kgnbdmwf	dfm-L"
  "dfm_L	2	checkpoints/dfm/L	epoch_2	2	hrm-dfm-L	logs/euroeval/dfm_L_epoch_checkpoints	DFM L	kgnbdmwf	dfm-L"
  "dfm_L	3	checkpoints/dfm/L	epoch_3	3	hrm-dfm-L	logs/euroeval/dfm_L_epoch_checkpoints	DFM L	kgnbdmwf	dfm-L"
  "dfm_L	4	checkpoints/dfm/L	epoch_4	4	hrm-dfm-L	logs/euroeval/dfm_L_epoch_checkpoints	DFM L	kgnbdmwf	dfm-L"
  "dfm4_XL	1	checkpoints/dfm4/XL-ddp	epoch_1	1	hrm-dfm4-XL-ddp	logs/euroeval/dfm4_XL_ddp_epoch_checkpoints	Original Plus Mixed Danish Instruction Rich L	dfm4xlddpcleanfixed2	dfm4-XL-ddp clean corrected history v2"
  "dfm4_XL	2	checkpoints/dfm4/XL-ddp	epoch_2	2	hrm-dfm4-XL-ddp	logs/euroeval/dfm4_XL_ddp_epoch_checkpoints	Original Plus Mixed Danish Instruction Rich L	dfm4xlddpcleanfixed2	dfm4-XL-ddp clean corrected history v2"
)
...
launch_job() {
  ...
  local family epoch ckpt_path ckpt_tag eval_epoch model_prefix log_base wandb_project wandb_run_id wandb_run_name
  IFS=$'\t' read -r family epoch ckpt_path ckpt_tag eval_epoch model_prefix log_base wandb_project wandb_run_id wandb_run_name <<< "${job_line}"
  local run_dir="${log_base}/${ckpt_tag}"
...
while (( next_job < ${#jobs[@]} || ${#running_pid_by_gpu[@]} > 0 )); do
  ...
  for gpu in "${GPUS_ARR[@]}"; do
    ...
    IFS=$'\t' read -r family epoch ckpt_path ckpt_tag _ <<< "${job_line}"
