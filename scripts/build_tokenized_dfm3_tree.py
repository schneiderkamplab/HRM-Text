#!/usr/bin/env python3
"""Build a symlinked tokenized tree for DFM3.

DFM3 reuses tokenized DFM/mixed tasks, DFM2 DynaWord-derived tasks, and adds
DFM3 Common Pile-derived English raw-text tasks. The sampler expects a single
tokenized root, so this script creates a symlink union without copying token
arrays.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=[
            Path("data/tokenized_mixed"),
            Path("data/tokenized_dfm2_dynaword_tasks"),
            Path("data/tokenized_dfm3_common_pile_tasks"),
        ],
    )
    parser.add_argument("--output", type=Path, default=Path("data/tokenized_dfm3"))
