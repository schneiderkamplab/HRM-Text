...
MODEL_NAME="${MODEL_NAME:-google/gemma-4-31B-it-fresh-20260604}"
DATA_ROOT="${DATA_ROOT:-$ROOT/data/mimir_benchmark_campaigns}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="${LOG_ROOT:-$ROOT/logs/mimir_benchmark_campaigns/$RUN_ID}"
...
worker_pids=()
cleanup() {
  mapfile -t campaign_clients < <(
    pgrep -f "^/home/ucloud/miniforge3/envs/hrm/bin/python scripts/mimir_benchmark_campaigns.py --data-root $DATA_ROOT " || true
  )
  if ((${#campaign_clients[@]})); then kill "${campaign_clients[@]}" 2>/dev/null || true; fi
...
requests_ready() {
  ...
  [[ "$(find "$DATA_ROOT/requests/shards" -maxdepth 1 -name 'part-*.jsonl' | wc -l)" -eq "$SHARDS" ]]
}
...
if ! requests_ready; then
  "$HRM_PYTHON" scripts/prepare_mimir_benchmark_campaigns.py --output "$DATA_ROOT"
fi
...
for gpu in $(seq 0 7); do
  ...
  setsid "$AUDIT_PYTHON" -m vllm.entrypoints.openai.api_server \
...
worker() {
  ...
  while claimed="$(claim "$gpu")"; do
    ...
    if {
      for pass in 1 2; do
        "$HRM_PYTHON" scripts/mimir_benchmark_campaigns.py --data-root "$DATA_ROOT" generate \
          --input "$shard" --output "$generated" \
          --base-url "http://127.0.0.1:${port}/v1" --model "$MODEL_NAME" \
          --concurrency "$CONCURRENCY" --max-tokens 2048
      done
      for pass in 1 2; do
        "$HRM_PYTHON" scripts/mimir_benchmark_campaigns.py --data-root "$DATA_ROOT" audit \
...
echo "Mimir benchmark campaigns finished: done=$done_markers failed=$failed_jobs"
(( failed == 0 && failed_jobs == 0 && done_markers == SHARDS ))
