...
INPUT_DIR="${INPUT_DIR:-data/converted_sources/code_meta_reasoning_repaired}"
AUDIT_DIR="${AUDIT_DIR:-logs/data_audits/code_meta_reasoning_repaired_20260828}"
...
if [[ ! -f "$AUDIT_DIR/inventory.json" ]]; then
  "$CLIENT_PYTHON" scripts/audit_repaired_code_meta_reasoning.py prepare \
    --input-dir "$INPUT_DIR" --audit-dir "$AUDIT_DIR" \
    --samples-per-family 100 --partitions 8
fi
...
run_partition() {
  ...
  echo "GPU${gpu}: starting code-meta audit partition ${partition}"
  ...
  for attempt in 1 2 3 4; do
    if "$CLIENT_PYTHON" scripts/audit_repaired_code_meta_reasoning.py audit \
...
"$CLIENT_PYTHON" scripts/audit_repaired_code_meta_reasoning.py merge \
  --audit-dir "$AUDIT_DIR" --partitions 8 --model "$SERVED_MODEL"
echo "Code meta-reasoning repaired audit complete"
