...
MODEL_NAME="${MODEL_NAME:-openai/gemma-4-e4b-opus-repaired-judge}"
...
printf '%s\n' "$PARTITIONS" > "$RUN_ROOT/partition_count.txt"
...
"$CLIENT_PYTHON" scripts/dfm10_quality_audit.py merge \
  ...
  --partitions "$PARTITIONS" --output "$RUN_ROOT/opus_da_en_repaired_quality_audit.jsonl"
