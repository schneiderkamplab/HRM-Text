...
GPUS_CSV="${GPUS_CSV:-0,1,2,5,6,7}"
CONCURRENCY_PER_SHARD="${CONCURRENCY_PER_SHARD:-128}"
MAX_NUM_SEQS="${MAX_NUM_SEQS:-128}"
MAX_ATTEMPTS="${MAX_ATTEMPTS:-3}"
LOG_ROOT="${LOG_ROOT:-$ROOT/logs/sapient_anonymization_recover_high40_then_repeat30_$(date +%Y%m%dT%H%M%S)}"
  ...
  (
    ...
    env PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES="$gpu" \
      python scripts/synthesize_anonymized_sapient_exclusions.py \
        ...
        --source-priority high40 \
        --concurrency "$CONCURRENCY_PER_SHARD" \
        > "$LOG_ROOT/workers/gpu${gpu}_high40_shard${gpu}of8.log" 2>&1
    echo "finished high40 gpu=$gpu shard=$gpu/8"
    ...
    echo "finished repeat30 gpu=$gpu shard=$gpu/8"
  ) &
  chain_pids+=("$!")
  echo "chain gpu=$gpu pid=${chain_pids[-1]}"
done
