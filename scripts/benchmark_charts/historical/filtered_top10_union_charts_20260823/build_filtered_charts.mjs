...
const isExcludedModel = (model) => /^(Qwen|Gemma|SmolLM3)/i.test(model);
const eligibleRecords = records.filter((record) => !isExcludedModel(record.model));
if (!eligibleRecords.some((record) => record.model === "Munin-Qwen3.5-9B")) {
  throw new Error("Munin-Qwen3.5-9B must remain eligible");
}
...
if (union.size !== 19) throw new Error(`Expected 19 union models, found ${union.size}`);
...
selectionSheet.getRange("A2:B6").values = [
  ["Included exception", "Munin-Qwen3.5-9B"],
...
selectionSheet.tables.add("A1:B6", true, "SelectionRules");
...
const allUnionFileNames = {
  english: "07_english_all19.png",
  danish: "08_danish_all19.png",
  math_code: "09_math_and_code_all19.png",
  overall: "10_overall_all19.png",
};

function addAllUnionCategoryChart(metric) {
  const metricLabel = metric.title.replace("Top 10 — ", "");
  const sheetName = `${metricLabel} — all 19`;
  ...
  const chart = sheet.charts.add("bar", {
    title: `Top-10 union (19) — ${metricLabel}`,
    titleTextStyle: { fontSize: 14 },
    hasLegend: false,
...
function addSizeChart() {
  const sheetName = "Size — 19 models";
  const sheet = workbook.worksheets.add(sheetName);
  sheet.showGridLines = false;
  ...
  const chart = sheet.charts.add("bar", {
    title: "Model size — dark: non-embedding, light: embedding (19-model union)",
    titleTextStyle: { fontSize: 14 },
    hasLegend: false,
    barOptions: { direction: "bar", grouping: "stacked", gapWidth: 48, overlap: 100 },
    xAxis: { axisType: "textAxis", orientation: "maxMin", textStyle: { fontSize: 10 } },
...
function addEfficiencyChart() {
  ...
  const chart = sheet.charts.add("bar", {
    title: "Overall / non-embedding size — 19-model union",
    titleTextStyle: { fontSize: 14 },
    hasLegend: false,
    barOptions: { direction: "bar", grouping: "stacked", gapWidth: 48, overlap: 100 },
    xAxis: { axisType: "textAxis", orientation: "maxMin", textStyle: { fontSize: 10 } },
...
await fs.writeFile(path.join(pngDir, "05_size_19_models.png"), new Uint8Array(await sizePreview.arrayBuffer()));
const efficiencyPreview = await workbook.render({ sheetName: efficiencyChart.sheet.name, range: "A1:P58", scale: 1.1, format: "png" });
await fs.writeFile(path.join(pngDir, "06_overall_per_noemb_19_models.png"), new Uint8Array(await efficiencyPreview.arrayBuffer()));
...
await xlsx.save(path.join(outputDir, "filtered_top10_union_charts.xlsx"));
