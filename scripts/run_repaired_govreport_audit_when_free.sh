...
AUDIT_DIR="${AUDIT_DIR:-logs/data_audits/govreport_summarization_repaired_20260828}"
...
echo "GPU${gpu}: starting repaired GovReport audit"
...
for attempt in 1 2 3 4; do
  if "$CLIENT_PYTHON" scripts/audit_repaired_govreport.py audit \
    --samples "$AUDIT_DIR/samples.jsonl" \
    --output "$AUDIT_DIR/results/audit.jsonl" \
...
"$CLIENT_PYTHON" scripts/audit_repaired_govreport.py summarize \
  --input "$AUDIT_DIR/results/audit.jsonl" --output "$AUDIT_DIR/summary.json"
